/**
 * a_bogus 签名算法 - 轻量版 (SM3 + RC4)
 * 
 * 来源: DouyinLiveWebFetcher 项目
 * 算法: SM3 哈希 + RC4 流加密
 * 
 * 使用方法:
 *   import { sign_detail, sign_reply } from './abogus_lite.js';
 *   const aBogus = sign_detail(queryString, userAgent);
 * 
 * 注意: 仅作为备用方案，推荐使用主方案 abogus.js
 */
...
function generate_rc4_bb_str(url_search_params, user_agent, window_env_str, suffix = "cus", args = [0, 1, 14]) {
    let sm3 = new SM3()
    ...
    // 对ua处理之后的结果
    let ua = sm3.sum(result_encrypt(rc4_encrypt(user_agent, String.fromCharCode.apply(null, [0.00390625, 1, args[2]])), "s3"))
...
/**
 * 生成 a_bogus 签名
 * @param {string} url_search_params - URL 查询参数字符串
 * @param {string} user_agent - User-Agent 字符串
 * @param {number[]} args - 签名参数，默认 [0, 1, 14]
 * @returns {string} a_bogus 签名
 */
export function sign(url_search_params, user_agent, args = [0, 1, 14]) {
...
/**
 * 生成详情接口签名
 * @param {string} params - URL 查询参数字符串
 * @param {string} userAgent - User-Agent 字符串
 * @returns {string} a_bogus 签名
 */
export function sign_detail(params, userAgent) {
 ...
 * @returns {string} a_bogus 签名
 */
export function sign_reply(params, userAgent) {
    return sign(params, userAgent, [0, 1, 8])
}

// 兼容旧名称
export const sign_datail = sign_detail;

// 导出内部函数 (供高级使用)
export { SM3, rc4_encrypt, result_encrypt };
